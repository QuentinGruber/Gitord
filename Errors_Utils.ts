...
*/
  ...
  Check_IssueMinimalBody(data, Body_size) {
    let Issues = data;
    var errors_found = [];
    if (Issues != undefined) {
      for (let i = 0; i < Issues.length; i++) {
        if (Issues[i].body == null && Issues[i].pull_request == undefined) {
          ...
          Issues[i].body.length < Body_size &&
          Issues[i].pull_request == undefined
        ) {
    ...
    if (Issues != undefined) {
      for (let i = 0; i < Issues.length; i++) {
        if (Issues[i].body == null) {
          ...
          String(Issues[i].body).includes("Close #") == false &&
  ...
  Check_PullNeedAssigneeWIP(data) {
    let Issues = data;
    var errors_found = [];
    if (Issues != undefined) {
      for (let i = 0; i < Issues.length; i++) {
        if (Issues[i].pull_request != undefined && Issues[i].assignee == null) {
          if (
            String(Issues[i].title).includes("WIP") ||
  ...
  Check_AssignedIssueNeedMstone(data) {
    let Issues = data;
    var errors_found = [];
    if (Issues != undefined) {
      for (let i = 0; i < Issues.length; i++) {
        if (
          Issues[i].assignee != null &&
          Issues[i].pull_request == undefined &&
